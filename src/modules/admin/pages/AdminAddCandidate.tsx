...
export function AdminAddCandidate() {
  ...
  function pickDoc(k: keyof DocState){ return (e:React.ChangeEvent<HTMLInputElement>)=>{ const f=e.target.files?.[0]; if(f) setDocs(d=>({...d,[k]:f.name})); }; }
  ...
  return (
    <>
      ...
      <main className="page">
        <PageTransition>
          <button onClick={()=>navigate('/admin/candidates')} className="mb-5 inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-secondary">
          ...
          <div className="mx-auto max-w-3xl">
            ...
            <motion.form initial={{opacity:0,y:16}} animate={{opacity:1,y:0}}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2"><Input label="Full name" placeholder="Candidate full name" error={errors.name?.message} {...register('name')}/></div>
                <div>
                ...
                <div className="sm:col-span-2"><Input label="Skills (comma separated)" placeholder="React, Node.js, AWS" error={errors.skills?.message} {...register('skills')}/></div>
                <div className="sm:col-span-2">
                  <label className="label">Summary</label>
                  <textarea rows={3} placeholder="Professional summary…" className={`field resize-none${errors.summary?' field-error':''}`} {...register('summary')}/>
                  ...
                  {([['Resume','resume'],['Evaluation','evaluationReport'],['Background','backgroundReport']] as const).map(([label,key])=>(
                    <label key={key} className="cursor-pointer">
                      <div className={`flex flex-col items-center gap-2 rounded-xl border-2 border-dashed p-4 text-center transition ${docs[key]?'border-success/40 bg-success/5':'border-slate-200 hover:border-primary hover:bg-primary-50'}`}>
                        <span className={`grid h-10 w-10 place-items-center rounded-lg ${docs[key]?'bg-success/10 text-success':'bg-slate-100 text-slate-400'}`}>{docs[key]?<FiCheck size={18}/>:<FiFile size={18}/>}</span>
                        <span className="text-sm font-medium text-secondary">{label}</span>
                        <span className="max-w-full truncate text-xs text-slate-400">{docs[key]||'Upload'}</span>
                      </div>
              ...
              <div className="mt-8 flex gap-3 sm:justify-end">
                ...
                <Button type="submit" loading={isSubmitting} className="sm:min-w-36">{isSubmitting?'Saving…':isEdit?'Save changes':'Add candidate'}</Button>
              </div>
            </motion.form>
          </div>
        </PageTransition>
