...
export function AdminRecruiters() {
  ...
  const { push }     = useToast();
  const registry     = useRef<User[]>([...seedUsers.filter(u => u.role === 'recruiter')]);
  const [recruiters, setRecruiters] = useState<User[]>(registry.current);
  ...
  async function onSubmit(v: CreateRecruiterValues) {
    ...
    if (editing) {
      ...
      if (recruiters.some(r => r.email.toLowerCase() === v.email.toLowerCase())) {
        push({ type:'error', title:'Email exists', description:'This email is already in use.' }); return;
      }
      const newR: User = { id:`rec-${Math.random().toString(36).slice(2,7)}`, role:'recruiter', ...v };
      setRecruiters(p => [newR, ...p]);
      push({ type:'success', title:'Recruiter created', description:`${v.firstName} ${v.lastName} added.` });
    }
  ...
  const filtered = recruiters.filter(r => {
    ...
    return !q || r.firstName.toLowerCase().includes(q) || r.lastName.toLowerCase().includes(q) || r.email.toLowerCase().includes(q) || r.companyName.toLowerCase().includes(q);
  });
  ...
  return (
    <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 text-left text-xs uppercase tracking-wider text-slate-400">
                    {['Recruiter','Company','Designation','Phone','Actions'].map(h => <th key={h} className="px-5 py-3 font-semibold">{h}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(r => (
                    <motion.tr key={r.id} initial={{ opacity:0 }} animate={{ opacity:1 }} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEdit(r)} title="Edit"
                            className="grid h-9 w-9 place-items-center rounded-lg text-slate-500 hover:bg-primary-50 hover:text-primary transition"><FiEdit2 size={15}/></button>
                          <button onClick={() => setToDelete(r)} title="Delete"
                            className="grid h-9 w-9 place-items-center rounded-lg text-slate-500 hover:bg-danger/10 hover:text-danger transition"><FiTrash2 size={15}/></button>
                        </div>
                      </td>
                    </motion.tr>
                  ))}
      ...
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); reset(); setEditing(null); }}
        title={editing ? 'Edit recruiter' : 'Create recruiter'} size="md">
